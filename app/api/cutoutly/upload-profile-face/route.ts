...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `cutoutly/avatars/${user.id}/${uuidv4()}_${file.name.replace(/\s+/g, "_")}`
    ...
    if (uploadError) {
      console.error("❌ Error uploading profile face image:", uploadError)
    ...
    console.error("❌ Error in upload-profile-face API:", error)
